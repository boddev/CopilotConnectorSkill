...
function extractJsonDocumentsFromValue(
  ...
  if (Array.isArray(value)) {
    const docs = value.map((item, index) => {
      ...
      return buildDocumentInfo({
        ...
        relativePath,
        body: cleanContent(jsonValueToText(item)),
  ...
  return [buildDocumentInfo({
    ...
    sourcePath: path.resolve(filePath),
    relativePath,
    body: cleanContent(jsonValueToText(value)),
    url: metadata.url || normalizeDocumentUrl(relativePath, urlPrefix),
  }, metadata)];
}
...
export function buildDocumentChunkContent(chunk: DocumentChunk, doc: DocumentInfo): string {
  ...
  lines.push("", "Content body:", chunk.text);
  return lines.join("\n");
}
...
export function displayValue(value: string, fieldName: string = "", config: EnhancerConfig | null = null): string {
  ...
  if (!raw) return raw;
  if (preservesNumericLiteral(fieldName, config)) return raw;
  if (/^-?\d+$/.test(raw)) {
    try {
...
export function factLine(fieldName: string, value: string, config: EnhancerConfig): string {
  const label = humanizeField(fieldName, config);
  if (label === fieldName) {
    return `- ${fieldName}: ${displayValue(value, fieldName, config)}`;
  }
  return `- ${fieldName} (${label}): ${displayValue(value, fieldName, config)}`;
}
...
export function buildRecordContent(opts: {
  ...
  if (rowNumber !== null) {
    lines.push(`Source data row: ${rowNumber} (1-based, excluding the header row)`);
  }
  if (entity) lines.push(`Entity: ${entity}`);
  if (year) lines.push(`Year or date: ${year}`);
  lines.push(`Record purpose: ${rowPurpose}`, "", "Key facts:");
  ...
  if (evalMatches.length > 0) {
    ...
    for (const item of evalMatches) {
      for (const field of Object.keys(item.supportingFacts)) {
        if (!isEmpty(rowValueForFact(row, field, config))) {
          fields.add(field);
        }
      }
    }
    ...
    if (sortedFields.length > 0) {
      lines.push("", "Evaluation-derived retrieval focus fields:");
      lines.push("- " + sortedFields.map(f => `${f} (${humanizeField(f, config)})`).join(", "));
    }
  }
...
export function run(args: RunArgs): Record<string, unknown> {
  ...
  for (const csvFile of csvFiles) {
    ...
    for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
      ...
      csvContent += writeCsvRow(item, csvFields);
